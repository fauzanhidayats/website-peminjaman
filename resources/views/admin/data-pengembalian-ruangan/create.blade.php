...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>Tambah Pengembalian Ruangan</h1>
                ...
                <div class="col-12 col-md-8 col-lg-8">
                    <div class="card">
                        <div class="card-header">
                            <h4>Form Tambah Pengembalian Ruangan</h4>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('pengembalian-ruangan.store') }}" method="POST">
                                @csrf

                                <div class="form-group">
                                    <label for="peminjaman_ruangan_id">Pilih Peminjaman Ruangan</label>
                                    <select name="peminjaman_ruangan_id"
                                        class="form-control @error('peminjaman_ruangan_id') is-invalid @enderror" required>
                                        <option value="">-- Pilih Peminjaman --</option>
                                        @foreach ($peminjamans as $peminjaman)
                                            <option value="{{ $peminjaman->id }}"
                                                {{ old('peminjaman_ruangan_id') == $peminjaman->id ? 'selected' : '' }}>
                                                {{ $peminjaman->user->username }} - {{ $peminjaman->ruangan->nama_ruangan }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('peminjaman_ruangan_id')
