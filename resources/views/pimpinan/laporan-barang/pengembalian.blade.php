...
@section('content')
    <section class="section">
        ...
        <form action="{{ route('pimpinan.laporan.pengembalian.barang.pdf') }}" method="GET" target="_blank">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @forelse ($pengembalian as $item)
                    <tr>
                        <td>{{ $item->peminjamanBarang->user->username ?? '-' }}</td>
                        <td>{{ $item->peminjamanBarang->barang->nama_barang ?? '-' }}</td>
