...
@section('content')
    <section class="section">
        ...
        <form action="{{ route('pimpinan.laporan.pengembalian.ruangan.pdf') }}" method="GET" target="_blank">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @forelse ($pengembalian as $item)
                    <tr>
                        <td>{{ $item->peminjamanRuangan->user->username ?? '-' }}</td>
                        <td>{{ $item->peminjamanRuangan->ruangan->nama_ruangan ?? '-' }}</td>
